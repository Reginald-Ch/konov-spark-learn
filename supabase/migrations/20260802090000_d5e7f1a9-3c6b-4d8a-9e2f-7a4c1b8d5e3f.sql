-- Deepens Module 1 (AI Foundations) per feedback: fundamentals need more
-- practical depth up front, and it's fine if the curriculum grows past 30
-- lessons to make room for that. Adds "Inside a Neural Network" as a new
-- Lesson 4, and writes full content + 7-question quizzes for the remaining
-- Module 1 stubs (How Computers "Learn", A Brief History, Where AI Shows Up
-- in Your Life, Meet the AI Careers). Module 2 gets the same depth pass next.
--
-- The order_index shift uses a temporary +1000 offset because the column has
-- a plain (non-deferred) UNIQUE constraint — shifting rows up by 1 in a
-- single UPDATE would collide mid-statement without it.
