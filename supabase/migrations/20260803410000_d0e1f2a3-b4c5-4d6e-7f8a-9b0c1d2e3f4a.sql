-- Complete Module 7 of the Lessons curriculum.
--
-- The live `lessons` table's module_number values are off by one from the
-- app's own MODULE_META labels (LessonsPanel.tsx): module_number 4 currently
-- holds "Build Your Own Chatbot" content, 5 holds "AI Agents & Tools", and 6
-- holds "Responsible & Ethical AI" (Bias, Privacy, AI's Impact on Society,
-- Your Path Forward) — while MODULE_META says 4="Code Fundamentals",
-- 5="Build Your Own Chatbot", 6="AI Agents & Tools", 7="Responsible &
-- Ethical AI". module_number 7 has zero rows. A prior attempt to fix this
-- (insert a real Module 4 "Code Fundamentals" and shift the rest up) was
-- written to this migrations folder but never actually reached the live
-- database — confirmed by querying the live table directly, not by trusting
-- migration history, since the two had already drifted apart.
--
-- This shifts the existing modules up by one (matching their real content to
-- their real label) and inserts the missing "Code Fundamentals" module as
-- the new Module 4, reusing the lesson content already written for that
-- earlier, never-applied attempt.

-- module_number has no uniqueness constraint, so a single UPDATE is safe
-- regardless of row order — lesson_progress/lesson_content/lesson_quiz_questions
-- all key off lesson_id (UUID), never module_number, so no student progress
-- is affected by this renumber.
UPDATE public.lessons SET module_number = module_number + 1 WHERE module_number IN (4, 5, 6);

-- order_index IS globally UNIQUE — shift everything from the current Module
-- "4" (now "5") onward by +6 to make room, using the same safe +1000/-994
-- offset trick already used elsewhere in this migration history (avoids
-- ever colliding with an existing value mid-update).
UPDATE public.lessons SET order_index = order_index + 1000 WHERE order_index >= 17;
UPDATE public.lessons SET order_index = order_index - 994 WHERE order_index >= 1017;

INSERT INTO public.lessons (module_number, order_index, title, slug, summary, coin_cost, is_published) VALUES
(4, 17, 'Variables & Values', 'variables-and-values', 'Storing information with named containers — the first building block of any program.', 10, true),
(4, 18, 'Data Structures: Lists & Dictionaries', 'data-structures-lists-dicts', 'Organizing more than one value at a time, the way real code (and your chatbot''s knowledge base) does.', 10, true),
(4, 19, 'Conditionals: Teaching Code to Decide', 'conditionals-if-else', 'if / elif / else — how code makes different decisions in different situations.', 10, true),
(4, 20, 'Functions: Reusable Logic', 'functions-reusable-logic', 'Writing logic once and reusing it everywhere, instead of copy-pasting.', 10, true),
(4, 21, 'Loops: Doing Things Automatically', 'loops-repeating-actions', 'Repeating an action across every item in a list, automatically.', 10, true),
(4, 22, 'F-Strings & String Formatting', 'f-strings-string-formatting', 'The shortcut for combining variables and text — used everywhere in the real FORGE IDE.', 10, true);
