-- Lessons system: 30 lessons across 6 modules, unlocked with Forge Coins,
-- completed via a 7-question quiz (5/7 to pass, unlimited retakes), with a
-- Forge Key awarded every 3rd lesson passed. Ties into the existing
-- point_events ledger (forge_coin_adjust for spending, forge_key for the
-- reward) rather than inventing a parallel currency system.
--
-- Content authoring for this pass: only Lesson 1 is fully written and
-- published, as a format sample. The remaining 29 are seeded as unpublished
-- stubs (title/module/summary only) so the full 30-lesson roadmap is visible
-- to participants immediately, without letting anyone spend coins unlocking
-- a lesson that has no real content yet.
...
-- No public write policy — content is seeded by migration for now; a proper
-- authoring UI is a later CMS pass, same as discussed for Resources.
...
-- Answers are readable directly (like daily_challenges.benchmark_tests) —
-- acceptable for a low-stakes learning quiz, not worth an RPC-only grading
-- indirection the way real prize-bearing scores are locked down elsewhere.
-- What DOES need locking down is completion/coin-spend/key-award, below.
...
-- No public policies at all — same pattern as reward_boxes. Only reachable
-- through the SECURITY DEFINER functions below, so unlocking/completing a
-- lesson (and the coin spend / key award that come with it) can't be forged
...
BEGIN
  ...
  -- A key every 3rd lesson passed — only fires the moment this attempt is
  -- what pushed them over a multiple of 3, never on retakes/re-passes.
...
-- ============================================================
-- Curriculum: 30 lessons, 6 modules. Only Lesson 1 is published with real
-- content — the rest are visible roadmap stubs until written.
-- ============================================================
