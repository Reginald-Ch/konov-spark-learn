...
const CHATBOT_SECTIONS: ConfigSection[] = [
  ...
  {
    id: 'personality', title: 'Personality', subtitle: 'How your bot thinks & talks', icon: <Sparkles className="w-4 h-4" />, color: '#F59E0B',
    fields: [
        ...
        <AnimatePresence>
          {showPreview && (
            <motion.div
              ...
              className="flex flex-col border-l border-[hsl(var(--discord-light)/0.12)] bg-[hsl(var(--discord-darker))] overflow-hidden md:max-w-[50%]"
              style={{ minWidth: showPreview ? '340px' : 0 }}
            >
              {/* Preview top bar */}
              <div className="px-4 py-2 border-b border-[hsl(var(--discord-light)/0.12)] flex items-center gap-2 bg-[hsl(var(--discord-dark))]">
                <Eye className="w-3.5 h-3.5 text-[hsl(var(--discord-text-muted))]" />
                <span className="text-[11px] font-bold text-[hsl(var(--discord-text-muted))] uppercase tracking-wider">Live UI Preview</span>
                <Badge className="ml-1 text-[8px] px-1.5 py-0 bg-[hsl(var(--discord-green)/0.15)] text-[hsl(var(--discord-green))] border-[hsl(var(--discord-green)/0.3)] animate-pulse">
                  LIVE
                </Badge>
                <div className="ml-auto flex items-center gap-1.5">
                  <Button
                    ...
                    className="h-7 px-2 text-[hsl(var(--discord-text-muted))] hover:text-white text-[10px] gap-1"
                  >
              ...
              <div className="flex-1 overflow-auto flex items-start justify-center p-4">
                <div className="w-full max-w-[380px] flex flex-col rounded-2xl border-2 border-[hsl(var(--discord-light)/0.2)] bg-[hsl(var(--discord-dark))] shadow-2xl shadow-black/40 overflow-hidden" style={{ height: 'min(600px, calc(100vh - 200px))' }}>
                  
                  {/* App Header */}
                  <div className="px-4 py-3 border-b border-[hsl(var(--discord-light)/0.1)] bg-gradient-to-r from-[hsl(var(--discord-dark))] to-[hsl(var(--discord-darker))]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="text-white font-bold text-sm">{config.BOT_NAME || 'My Bot'}</div>
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-2 rounded-full bg-[hsl(var(--discord-green))] animate-pulse" />
                          <span className="text-[10px] text-[hsl(var(--discord-green))]">Online</span>
                          {builderType === 'agent' && config.TOOLS?.length > 0 && (
                            <span className="text-[9px] text-[hsl(var(--discord-text-muted))] ml-1">• {config.TOOLS.length} tools</span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 rounded-full bg-[hsl(var(--discord-text-muted)/0.3)]" />
                        <div className="w-2 h-2 rounded-full bg-[hsl(var(--discord-text-muted)/0.3)]" />
                        <div className="w-2 h-2 rounded-full bg-[hsl(var(--discord-text-muted)/0.3)]" />
                      </div>
                    </div>
                    {config.CREATOR_NAME && config.CREATOR_NAME !== 'A FORGE Builder' && (
                      <div className="mt-1.5 text-[9px] text-[hsl(var(--discord-text-muted))]">by {config.CREATOR_NAME}</div>
                    )}
                  </div>
                    ...
                    {config.AI_MESSAGE && chatMessages.length === 0 && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2.5">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="bg-[hsl(var(--discord-darker))] rounded-2xl rounded-tl-md px-3.5 py-2.5 inline-block max-w-[85%]">
                            <p className="text-[13px] text-white leading-relaxed">{config.AI_MESSAGE}</p>
                          </div>
                        </div>
                      </motion.div>
                    )}
