...
"""
🤖 AI Chatbot — Production-Ready Conversational AI
Built with LangChain + Streamlit

Features:
  • Persistent chat history with ConversationBufferWindowMemory
  • Configurable system prompt, model, and temperature
...
MEMORY_WINDOW = int(os.getenv("MEMORY_WINDOW", "20"))  # Remember last N exchanges
...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        value=API_KEY,
        type="password",
        help="Enter your OpenAI API key. It's never stored on disk.",
    )
    ...
    system_prompt = st.text_area(
        ...
        height=150,
        help="Controls your chatbot's personality and behaviour",
    )
...
# Build LLM instance
@st.cache_resource
def get_llm(_api_key, _model, _temp, _max_tok):
    """Create a cached LLM instance (re-created when params change)."""
    return ChatOpenAI(
        model_name=_model,
        temperature=_temp,
        max_tokens=_max_tok,
...
def build_chain(llm, memory, sys_prompt):
...
# ──────────────────────────────────────────────
st.title("🤖 AI Chatbot")
st.caption("Powered by LangChain • Conversation memory enabled")
...
if user_input := st.chat_input("Type your message..."):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            chain = build_chain(llm, st.session_state.memory, system_prompt)
...
Features:
  • ReAct agent with tool-calling (search, calculator, Wikipedia)
  • Streaming thought process display
  • Conversation history with context window
  • Error recovery and graceful fallbacks
  • Configurable tools and model settings
"""
...
When given a task:
1. Think step-by-step about what tools to use
2. Execute tools to gather information
3. Synthesise results into a clear answer
Always cite your sources when using search or Wikipedia."""
...
# ──────────────────────────────────────────────
def create_tools():
    """Initialise all available tools for the agent."""
    tools = []
    ...
    python_repl = PythonREPLTool(
        ...
        description="Run Python code for maths, data processing, or analysis. Input should be valid Python.",
    )
    ...
    wiki_tool = Tool(
        name="wikipedia",
        func=wiki.run,
        description="Look up factual information on Wikipedia. Good for history, science, people, places.",
    )
...
with st.sidebar:
    ...
    if st.session_state.get("agent_history"):
        st.caption(f"📊 {len(st.session_state.agent_history)} messages")
...
def get_agent(_api_key, _model):
    """Create a cached agent instance."""
    llm = ChatOpenAI(
        model_name=_model,
        temperature=0,
        openai_api_key=_api_key,
...
# ──────────────────────────────────────────────
st.title("🧠 AI Agent")
st.caption("Autonomous AI with web search, calculator, and Wikipedia")
...
if task := st.chat_input("Give me a task..."):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            if show_reasoning:
                # Show the agent's thought process in real-time
  ...
  "max_iterations": 5,
  "capabilities": ["web_search", "calculator", "code_execution", "wikipedia"]
}`,
    requirements: `streamlit>=1.28.0
langchain>=0.3.0
langchain-openai>=0.2.0
